...
import { studentMedicalRecords } from '../../../lib/db/schema.js';
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  const { studentId, schoolId, type, title, description, severity, date } = body;
...
export const DELETE: APIRoute = async ({ request, locals }) => {
  ...
  const db = getDb();
  db.delete(studentMedicalRecords).where(eq(studentMedicalRecords.id, id)).run();
