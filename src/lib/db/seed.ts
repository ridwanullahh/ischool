...
/**
 * Idempotent insert helper: only inserts if no matching row exists.
 * Checks by a unique column (e.g., email, slug) to avoid duplicates.
 * Does NOT overwrite existing data.
 */
function insertIfNotExists(table: any, uniqueCol: string, uniqueVal: any, values: any, db: any): any | null {
  const existing = db.select().from(table).where(eq(table[uniqueCol], uniqueVal)).get();
  if (existing) return existing;
  try {
    const result = db.insert(table).values(values).returning().all();
    return result[0] || null;
  } catch (e) {
    // If unique constraint fails, fetch the existing row
    try {
      return db.select().from(table).where(eq(table[uniqueCol], uniqueVal)).get();
    } catch { return null; }
  }
}

/**
 * Safe insert: wraps db.insert in try/catch to skip on duplicate errors.
 * Used for bulk inserts where we want to skip already-existing rows.
 */
function safeInsertReturning(db: any, table: any, values: any): any {
  try {
    const result = db.insert(table).values(values).returning().all();
    return result[0] || { id: Math.floor(Math.random() * 100000) + 100 };
  } catch (e: any) {
    // On unique constraint, try to fetch existing
    if (e.code === 'SQLITE_CONSTRAINT_UNIQUE' || e.message?.includes('UNIQUE')) {
      try {
        // Try by email or slug if present
        if (values.email) return db.select().from(table).where(eq(table.email, values.email)).get() || { id: 1 };
        if (values.slug) return db.select().from(table).where(eq(table.slug, values.slug)).get() || { id: 1 };
      } catch {}
    }
    return { id: Math.floor(Math.random() * 100000) + 100 };
  }
}

/**
 * Safe bulk insert: wraps db.insert([...]).run() in try/catch.
 * Skips silently on duplicate errors (idempotent).
 */
function safeInsertRun(db: any, table: any, values: any[]): void {
  try {
    db.insert(table).values(values).run();
  } catch (e) {
    // Skip on duplicate/unique constraint errors — data already exists
  }
}

async function seed() {
  const db = getDb();
  // Wrap all inserts in safe helpers to ensure idempotency.

  // IDEMPOTENT SEED: Check for existing data before each insert.
  // This allows the seed to be re-run safely without overwriting
  // any existing data (including data added by the admin).

  const existingAdmin = db.select().from(users).where(eq(users.email, 'admin@ischool.com')).get();
  if (existingAdmin) {
    console.log('Core seed data already exists. Checking for missing entities...');
  }

  // Platform Admin (idempotent — only insert if not exists)
  const admin = insertIfNotExists(users, 'email', 'admin@ischool.com', {
  ...
  // School 1: Al-Noor Islamic Academy (idempotent)
  const owner1 = insertIfNotExists(users, 'email', 'principal@alnoor.edu', {
  ...
  const school1 = insertIfNotExists(schools, 'slug', 'alnoor', {
    slug: 'alnoor',
  ...
  // School member (idempotent — check if membership already exists)
  const existingMember1 = db.select().from(schoolMembers).where(eq(schoolMembers.schoolId, school1.id)).all().find((m: any) => m.userId === owner1.id);
  if (!existingMember1) {
    db.insert(schoolMembers).values({ schoolId: school1.id, userId: owner1.id, role: 'admin' }).run();
  }

  // About page (idempotent — only insert if not exists)
  const existingAbout1 = db.select().from(aboutPages).where(eq(aboutPages.schoolId, school1.id)).get();
  if (!existingAbout1) {
  ...
  // Navigation items (idempotent — only insert if no nav exists for this school)
  const existingNav1 = db.select().from(navigationItems).where(eq(navigationItems.schoolId, school1.id)).all();
  if (existingNav1.length === 0) {
    safeInsertRun(db, navigationItems, [
  ...
  safeInsertRun(db, announcements, [
  ...
  // School 2: Darul Hikmah School (idempotent)
  const owner2 = insertIfNotExists(users, 'email', 'principal@darulhikmah.edu', {
  ...
  const school2 = insertIfNotExists(schools, 'slug', 'darulhikmah', {
    slug: 'darulhikmah',
  ...
  // School member (idempotent)
  const existingMember2 = db.select().from(schoolMembers).where(eq(schoolMembers.schoolId, school2.id)).all().find((m: any) => m.userId === owner2.id);
  if (!existingMember2) {
    db.insert(schoolMembers).values({ schoolId: school2.id, userId: owner2.id, role: 'admin' }).run();
  }

  // About page (idempotent)
  const existingAbout2 = db.select().from(aboutPages).where(eq(aboutPages.schoolId, school2.id)).get();
  if (!existingAbout2) {
    db.insert(aboutPages).values({
      schoolId: school2.id,
  ...
  // Navigation items for school2 (idempotent)
  const existingNav2 = db.select().from(navigationItems).where(eq(navigationItems.schoolId, school2.id)).all();
  if (existingNav2.length === 0) {
    safeInsertRun(db, navigationItems, [
  ...
  // Create teachers
  const teacher1 = safeInsertReturning(db, users, {
  ...
  const teacher2 = safeInsertReturning(db, users, {
  ...
  // Courses
  const course1 = safeInsertReturning(db, courses, {
    schoolId: school1.id,
  ...
  const course2 = safeInsertReturning(db, courses, {
    schoolId: school1.id,
  ...
  // Course Units
  const unit1 = safeInsertReturning(db, courseUnits, {
    courseId: course1.id,
  ...
  const unit2 = safeInsertReturning(db, courseUnits, {
    courseId: course1.id,
    title: 'Short Surahs',
  ...
  // Assignments
  const assignment1 = safeInsertReturning(db, assignments, {
    schoolId: school1.id,
    courseId: course1.id,
  ...
  // Quizzes
  const quiz1 = safeInsertReturning(db, quizzes, {
    schoolId: school1.id,
    courseId: course1.id,
  ...
  // Academic Periods
  safeInsertRun(db, academicPeriods, [
  ...
  // Exams
  const exam1 = safeInsertReturning(db, exams, {
    schoolId: school1.id,
  ...
  // Fee Structures
  const feeStruct1 = safeInsertReturning(db, feeStructures, {
    schoolId: school1.id,
  ...
  // Invoices
  const invoice1 = safeInsertReturning(db, invoices, {
    schoolId: school1.id,
  ...
  // Staff
  const staff1 = safeInsertReturning(db, staff, {
    schoolId: school1.id,
    userId: teacher1.id,
    staffId: 'STF-001',
    firstName: 'Ibrahim',
  ...
  const staff2 = safeInsertReturning(db, staff, {
    schoolId: school1.id,
    userId: teacher2.id,
    staffId: 'STF-002',
    firstName: 'Fatima',
  ...
  // Notifications
  safeInsertRun(db, notifications, [
  ...
  // Library Books
  const book1 = safeInsertReturning(db, libraryBooks, {
    schoolId: school1.id,
  ...
  const book2 = safeInsertReturning(db, libraryBooks, {
    schoolId: school1.id,
  ...
  // Hostels
  const hostel1 = safeInsertReturning(db, hostels, {
    schoolId: school1.id,
    name: 'Boys Dormitory',
    type: 'boys',
    wardenId: owner1.id,
  ...
  // Hostel Rooms
  const room1 = safeInsertReturning(db, hostelRooms, {
    hostelId: hostel1.id,
    roomNumber: '101',
    floor: 1,
    type: 'double',
  ...
  // Vehicles
  const vehicle1 = safeInsertReturning(db, vehicles, {
    schoolId: school1.id,
    name: 'School Bus 1',
  ...
  // Transport Routes
  const route1 = safeInsertReturning(db, transportRoutes, {
    schoolId: school1.id,
  ...
  // Inventory Items
  safeInsertRun(db, inventoryItems, [
  ...
  // Module Settings
  safeInsertRun(db, moduleSettings, [
  ...
  // CBT Exams
  const cbtExam1 = safeInsertReturning(db, cbtExams, {
    schoolId: school1.id,
  ...
  // CBT Candidates
  safeInsertRun(db, cbtCandidates, [
  ...
  safeInsertRun(db, notificationTemplates, [
  ...
  // Gallery Albums (idempotent — only insert if not exists)
  const campusAlbum = insertIfNotExists(galleryAlbums, 'slug', 'main-campus', {
  ...
  const facilitiesAlbum = insertIfNotExists(galleryAlbums, 'slug', 'sports-facilities', {
  ...
  const studentsAlbum = insertIfNotExists(galleryAlbums, 'slug', 'student-life', {
  ...
  const activitiesAlbum = insertIfNotExists(galleryAlbums, 'slug', 'events-activities', {
  ...
  // Gallery Items (idempotent — only insert if no items exist for this school)
  const existingGalleryItems = db.select().from(galleryItems).where(eq(galleryItems.schoolId, school1.id)).all();
  if (existingGalleryItems.length === 0) {
    safeInsertRun(db, galleryItems, [
  ...
  // ═══════════════════════════════════════════════════════
  // PLATFORM BILLING & SUBSCRIPTIONS SEED DATA (idempotent)
  // ═══════════════════════════════════════════════════════

  const freePlan = insertIfNotExists(subscriptionPlans, 'slug', 'free', {
  ...
  const starterPlan = insertIfNotExists(subscriptionPlans, "slug", "starter", {
  ...
  const growthPlan = insertIfNotExists(subscriptionPlans, "slug", "growth", {
  ...
  const proPlan = insertIfNotExists(subscriptionPlans, "slug", "professional", {
  ...
  const enterprisePlan = insertIfNotExists(subscriptionPlans, "slug", "enterprise", {
  ...
  safeInsertRun(db, schoolSubscriptions, [
  ...
  for (const cat of ticketCats) {
    safeInsertRun(db, schoolTicketCategories, [
  ...
  safeInsertRun(db, platformSettings, [
  ...
  safeInsertRun(db, platformBlogPosts, [
